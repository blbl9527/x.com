...
<div class="row">
    ...
    <div class="row">
	    ...
	    <h4>工作详情</h4>
			<table class="table table-hover table-bordered">
				   ...
				   <tbody>
				      ...
				      <tr>
				         <td>{{$data['id']}}</td>
				         <th><a href="{{$data['abtpurl']}}" target="blank">{{$data['name']}}</a></th>
				         <td>{{$data['work']}}</td>
				         <td>{{$data['time']}}</td>
				         <td>{{$data['salary']}}</td>
				         <td>{{$data['area']}}</td>
        ...
        <form action="{{route('service.update',[$data['id']])}}" method="POST">
            ...
            <input type="text" name="d" value="d" hidden>
            <input type="submit" class="btn btn-danger" value="完成此次交易">
       </form>
       <form action="{{url('/consumer')}}" method="GET">
          <input type="submit" class="btn btn-success" value="我要再想想看">
       </form>
       </div> 
